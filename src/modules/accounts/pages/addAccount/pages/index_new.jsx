...
const AddAccount = () => {
  ...
  const {
    ...
    thirdPartyExportedAccountDataLoading,
  } = useSelector((state) => ({
  ...
  useEffect(() => {
    dispatch(
      ...
      if (res?.payload?.data?.accounts?.length > 0) {
        let Arr = res?.payload?.data?.accounts?.map((account) => ({
          value: account?.id,
          label: `${account?.name} (${account?.csn})`,
          bdPerson: account?.user_assign_firstname || [],
        }));
  ...
  const handleAccount = (selectedOption) => {
    ...
    const bdPersonLabels = isBDSelected
      ? selectedOption.map((item) => item?.label)
      : [];

    const filteredData = thirdPartyExportedAccountData
      ?.filter((account) => {
        if (!isBDSelected) return true;
        return account?.user_assign_Arr?.some((label) =>
          bdPersonLabels.includes(label)
        );
      })
      ?.map((account) => ({
        value: account?.id,
          ...
          <div className="add-account-form">
            ...
            <form className="">
              ...
              <CommonButton
                ...
                isDisabled={
                  isSubmitting || thirdPartyExportedAccountDataLoading
                }
              >
                {id
                  ? isSubmitting
